...
public class ThorpedeBehaviour : MonoBehaviour {
	...
	private void CalculateAttackIconPosition()
	{
		...
		if(target!=null)
		{
			...
			if(Mathf.Abs(between)>=Abilities.ThorpedeParameters.maxTurnAngle)
			{
				if(between>0)
					nnewAngle=Mathf.Repeat(angle-Abilities.ThorpedeParameters.maxTurnAngle,360);
				else
					nnewAngle=Mathf.Repeat(angle+Abilities.ThorpedeParameters.maxTurnAngle,360);
			}
			else
			...
			if(cos>=0)
				movePoint=Quaternion.Euler(0,0,-nnewAngle)*new Vector2(0,1)*Abilities.ThorpedeParameters.maxRange;
			else
				movePoint=Quaternion.Euler(0,0,-nnewAngle)*new Vector2(0,1)*Abilities.ThorpedeParameters.minRange;
	...
	private void CalculatePath()
	{
		if(!GameStorage.getInstance().isRunning)
		{
			...
			point2=Quaternion.Euler(0,0,-angle)*new Vector2(0,Abilities.ThorpedeParameters.minRange*Mathf.Abs(getAngleDst(angle,getAttackIconAngle())/Abilities.ThorpedeParameters.maxTurnAngle)*Vector2.Distance(point1,point4)/Abilities.ThorpedeParameters.maxRange)*Abilities.ThorpedeParameters.lowerSmooth;
			point2+=point1;
			...
			point3 = new Vector2(pointz.y,-pointz.x)*getAngleDst(angle,getAttackIconAngle())/Abilities.ThorpedeParameters.maxTurnAngle*Vector2.Distance(point1,point4)/Abilities.ThorpedeParameters.maxRange*Abilities.ThorpedeParameters.upperSmooth;
